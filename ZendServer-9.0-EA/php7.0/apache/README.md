...
To build run:
```
docker build -t janatzend/zend-server:9.0EA-php7.0 .
```
from within the cloned directory (please note the trailing dot).

To run:
```
docker run -d -P janatzend/zend-server:9.0EA-php7.0
```
...
```
docker run -d -p 88:80 -p 10088:10081 janatzend/zend-server:9.0EA-php7.0
```
...
```
docker run -d janatzend/zend-server:9.0EA-php7.0
```
or
```
docker run janatzend/zend-server:9.0EA-php7.0
```
...
```
docker run -e INSTALL_MYSQL=true -e MYSQL_PASSWORD=<password> -e MYSQL_USERNAME=<username> janatzend/zend-server:9.0EA-php7.0
```
...
```
docker run -e MYSQL_HOSTNAME=<db-ip> -e MYSQL_PORT=3306 -e MYSQL_USERNAME=<username> -e MYSQL_PASSWORD=<password> -e MYSQL_DBNAME=zendserver zend-server:9.0EA-php7.0
```
